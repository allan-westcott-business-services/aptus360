-- ════════════════════════════════════════════════════════════════
-- 0167 — the bill orders what is in force, and itemises gas by size
--
-- ── The size in force ──
--
-- Every utility line carries two sizes: what the build worked out, and
-- what a designer overrode it with. sizeMode.js has always read the
-- override where there is one and the calculated size everywhere else —
-- "the sizes that would be built" — and the bill read only the
-- calculated one.
--
-- So a length overridden to 180mm was ordered as the 125mm the build had
-- worked out, and nothing on the sheet said otherwise. The drawing, the
-- levels check and the bill were three answers to one question.
--
-- ── Gas, itemised ──
--
-- Gas mains came out as one row called "Gas Main" with no size, so a
-- scheme carrying 180mm, 125mm and 90mm was a single quantity of metres
-- nobody could order against. Water has been split by diameter since
-- 0117 and gas has the same catalogue — it was never joined.
--
-- Both changes are in the lines CTE. The point counts, the developer
-- split, the item names and the ordering are untouched.
--
-- Built from the deployed definition in 0126.
-- ════════════════════════════════════════════════════════════════
...
-- ── Check ───────────────────────────────────────────────────────
--
-- The gas mains, now by size. Expect one row per diameter rather than a
-- single "Gas Main":
--
--   SELECT item, quantity, features FROM gis_bom(<project id>)
--    WHERE item ILIKE 'gas main%' ORDER BY item;
--
-- ** What the override changed. ** Lengths where a designer overrode the
-- build. Before this, every one of them was ordered at the other size:
--
--   SELECT f."Layer_Key",
--          f."Attributes" ->> 'Manual_VD_Cable_Size_ID'  AS manual_cable,
--          f."Attributes" ->> 'VD_Cable_Size_ID'         AS system_cable,
--          f."Attributes" ->> 'Manual_Gas_Pipe_Size_ID'  AS manual_gas,
--          f."Attributes" ->> 'Gas_Pipe_Size_ID'         AS system_gas,
--          f."Attributes" ->> 'Manual_Water_Pipe_Size_ID' AS manual_water,
--          f."Attributes" ->> 'Water_Pipe_Size_ID'        AS system_water,
--          ROUND((f."Attributes" ->> 'Length_m')::numeric, 1) AS metres
--     FROM "GIS_Feature" f
--    WHERE f."Project_ID" = <project id>
--      AND f."Feature_Type" = 'line'
--      AND (f."Attributes" ? 'Manual_VD_Cable_Size_ID'
--        OR f."Attributes" ? 'Manual_Gas_Pipe_Size_ID'
--        OR f."Attributes" ? 'Manual_Water_Pipe_Size_ID');
--
-- A size the catalogue does not hold shows as "(pipe size not in the
-- catalogue)" rather than falling into the unset row. Anything here is
-- a feature pointing at a row somebody has deleted:
--
--   SELECT item, quantity FROM gis_bom(<project id>)
--    WHERE item ILIKE '%not in the catalogue%';
