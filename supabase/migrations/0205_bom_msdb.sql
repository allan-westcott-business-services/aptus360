-- The MSDB on the bill of materials: the board, and the tails in it.
--
-- Rebuilt from 0204 rather than patched, because gis_bom is one
-- function and there is no replacing half of it. The body below is
-- 0204's, with two additions and nothing else touched — an earlier
-- attempt at this rewrote the function from memory and lost the site,
-- utility and developer columns, the surface handling and most of the
-- water pipe cases. A working function is copied, not recalled.
--
-- ── 1. The board is a unit ──
--
-- With no name of its own it fell through to initcap('msdb') and read
-- "Msdb" on the sheet. It is an MSDB everywhere else in the app.
--
-- ── 2. The tails are cable somebody has to buy ──
--
-- A board's flats are rows in `MSDB_Apartments`, not lines on the
-- drawing: one cable arrives at the board and the dwellings hang off it
-- inside the building. Nothing was counting them, so the take-off was
-- short by however many metres of riser the block needs.
--
-- Reported as their own line rather than folded into the drawn service
-- cable of the same size. They are ordered together but cut, pulled and
-- terminated differently, and somebody asked "how much of that is in
-- the risers" should be able to read the answer rather than derive it.
--
