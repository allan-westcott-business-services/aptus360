      ...
      <div className={isTrench ? "fe fe-wide"
        ...
        <div className="fe-body">
          ...
          {feature.Feature_Role === "poc" && feature.Layer_Key === "electric" && (<>
            ...
            {/* ── The supply cable into the substation ──

                An HV run from here to the substation is not a circuit:
                it carries no meters, so the circuit colours below never
                reach it and it draws in the HV line type's red. Asked
                for, after setting a circuit colour left the cable
                leaving the POC unchanged.

                Kept on the POC, not on the cable. The route is
                generated by Route from POC, so a colour written on the
                cable would be lost the next time that ran; the rebuilt
                cable finds this one again by naming its POC.

                Shown only where such a route exists, with its own
                explanation — a POC feeding only LV has no supply cable
                to colour. */}
            {feature.Feature_Role === "poc" && (allFeatures || []).some((x) =>
              x.Attributes?.Poc_Route
              && Number(x.Attributes?.Poc_Route_Poc_ID) === Number(feature.Feature_ID)) && (
              <div className="fld">
                <label htmlFor="poc-route-colour">Supply cable colour</label>
                <div className="fe-circ-colour">
                  <span className="fe-circ-name">Route to the substation</span>
                  <label className="fe-swatch" title="Colour of the HV run from here">
                    <input id="poc-route-colour" type="color"
                      value={f.Attributes?.Route_Colour || "#b91c1c"}
                      onChange={(e) => setAttr("Route_Colour")(e.target.value)} />
                    <span style={{ background: f.Attributes?.Route_Colour || "#b91c1c" }} />
                  </label>
                  {f.Attributes?.Route_Colour && (
                    <button type="button" className="fe-clear"
                      onClick={() => setAttr("Route_Colour")("")}>use the HV colour</button>
                  )}
                </div>
                <p className="hint">
                  The cable from this point of connection to the substation. Saved
                  with Save, and kept when the route is rebuilt.
                </p>
              </div>
            )}
...
.fe-clear { border: 0; background: none; padding: 0; font: inherit;
