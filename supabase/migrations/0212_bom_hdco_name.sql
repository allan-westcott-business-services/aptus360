-- ════════════════════════════════════════════════════════════════
-- 0212 — the bill names a heavy duty cut-out HDCO
--
-- The point items on the bill are named from a list rather than from
-- initcap, because there is no rule that turns 'poc' into "POC" and
-- 'servicevalve' into "Service Valve" — those are facts about the
-- trade, not about the string.
--
-- 'hdcutout' was never added to that list, so it fell through to the
-- fallback and came out "Hdcutout": the role key wearing a capital
-- letter. It is HDCO on every schedule, every call-off and every van.
--
-- The whole function is replaced rather than patched, because a
-- Postgres function has no way to amend one branch of one CASE. This
-- is 0207 with one line added, and nothing else changed.
-- ════════════════════════════════════════════════════════════════
