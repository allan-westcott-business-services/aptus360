/* Which plots a booking may take, once a phase is split by utility.
   ...
   for the electric: different trench, different gang, often a different
   week. That is the whole point of splitting a phase. But two bookings
   covering the same utility cannot both have plot four, or whoever
   turns up second finds the work done \u2014 or worse, both dig.
...
/* Six plots, all laid for gas and water by one team. */
const existing = [{
  Assignment_ID: 1, Task_Type_ID: 1, Team_ID: 5, Plot_Range: "1-6",
  Start_Date: "2026-08-10", End_Date: "2026-08-11",
}];
const utilitiesOf = (a) => (Number(a.Assignment_ID) === 1 ? GAS_WATER : []);
const taken = (mine) => [...takenPlots(existing, 1, null, () => null,
  { utilitiesOf, mine })].map(([p]) => p);

// 1. The electric is free to take all six.
if (taken(ELECTRIC).length) {
  fail(`electric was refused plots ${taken(ELECTRIC).join(", ")}`);
}

// 2. More gas and water is not.
if (taken(GAS_WATER).length !== 6) {
  fail(`a second gas/water booking was offered ${6 - taken(GAS_WATER).length} of the six`);
}
...
if (!clashes(GAS_WATER).length) fail("save allowed a second gas/water booking on the same plots");
...
console.log(bad ? `\n${bad} problem(s)`
  : "Utility split behaves (electric may take plots the gas and water hold).");
