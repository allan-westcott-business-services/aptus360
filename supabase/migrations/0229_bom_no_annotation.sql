-- ════════════════════════════════════════════════════════════════
-- 0229 — the bill stops counting the writing on the drawing
--
-- A bill of materials is read by somebody ordering against it, and
-- two of the things on these drawings are not things that arrive on a
-- lorry:
--
--   'textnote'     a note written on the plan (0228). It is words.
--   'sectionmark'  the mark saying where a section is taken (0214).
--     A bar and two heads, drawn two metres long so it reads at scale.
--
-- Both are ANNOTATION — notes about the job rather than parts of it —
-- and 0215 already says so in as many words when it gives them a layer
-- of their own. The bill had not been told. Every point whose role is
-- not on the exclusion list is counted, so a drawing with three notes
-- and two section marks on it billed "Textnote 3 no." and
-- "Sectionmark 2 no." beside the ducting.
--
-- ── The section mark is a correction, not a new rule ──
--
-- It has been counted since 0214 shipped, on every project where
-- somebody placed one. Running this will change the bill on those
-- projects: the rows disappear, and the totals beneath them drop by
-- the count. That is the bill becoming right rather than changing its
-- mind, but it is a visible change to a document people have already
-- read, so it is said here rather than left to be discovered.
--
-- To see which projects this affects before running it:
--
--   SELECT "Project_ID", "Feature_Role", COUNT(*)
--     FROM "GIS_Feature"
--    WHERE "Feature_Role" IN ('sectionmark', 'textnote')
--    GROUP BY 1, 2 ORDER BY 1;
--
-- ── Why the whole function again ──
--
-- A Postgres function has no way to amend one line of one WHERE
-- clause, so it is replaced wholesale. This is 0212 with two roles
...
-- ── Check ───────────────────────────────────────────────────────
-- Expect no notes and no section marks on the bill:
...
-- seeds, span nodes, feeder end points, supplies and annotation. The
-- list below is the same one the function uses, and the two have to
-- stay in step or this query stops testing anything. Expect no rows:
