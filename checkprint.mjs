...
/* ── Cases 7 to 9 have gone with the path they tested ──

   They held the browser print together: the sheet drawn by the canvas
   into an image, placed in a print window sized in millimetres, with
   its own scale bar, its own fit-to-page warning and its own route to
   the browser's dialogue.

   Printing is a PDF now \u2014 vector features over the basemap's own
   embedded page \u2014 so none of those things exist to test. What replaced
   them is tested in checkprinttiles (how many sheets, of which paper,
   with what overlap) and checkprintpdf (to scale, one page per sheet,
   vector, and in register with the basemap).

   The arithmetic above survives because it is the same arithmetic: a
   metre on the ground is 1000/N millimetres on paper however the
   millimetres are put there. It is what both the old path and the new
   one rest on, which is why it is kept here rather than deleted with
   the rest. */

console.log(bad ? `\n${bad} problem(s)`
  : "Printing is to scale (the arithmetic a sheet is measured by).");
