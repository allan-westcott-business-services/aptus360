...
/* ── A joint is NOT adopted as a feeder end point ──

   It was, briefly, and it was the wrong shape. A straight joint is a
   fitting AND there is a stop on the run where it sits, but they are
   two objects: the diamond says what is in the ground, the circle says
   where this is on the cable, and a designer wants to move each
   without the other. Adopting the joint fused them into one thing that
   could only be dragged together.

   A breech has had it right all along: the fitting is a joint and the
   build makes a separate feeder point beside it. A straight joint works
   the same way now — `jointMarks` offers the stop, and nothing adopts
   it, so the build creates a feeder point there like any other. */
...
function writeFor(f, { num, label, kind, circuit }) {
  ...
  const named = f.Feature_Role === "linkbox";
