-- ════════════════════════════════════════════════════════════════
-- 0229 — the bill stops counting text notes
--
-- A bill of materials is read by somebody ordering against it, and a
-- note written on the drawing is not a thing that arrives on a lorry.
-- It is words.
--
-- `gis_bom` counts every point whose role is not on an exclusion list,
-- and 0228 adds a role. Without this, a drawing with three notes on it
-- would bill "Textnote 3 no." beside the ducting.
--
-- ── This changes no bill that exists ──
--
-- 0228 is the migration that makes a text note possible, so on the day
-- this is run there are none: every existing bill comes out exactly as
-- it did. That is the reason this one is on its own — see below.
--
-- ── The cross-section mark has the same fault and is NOT fixed here ──
--
-- 'sectionmark' (0214) is annotation by the same argument: a bar and
-- two heads saying where a section is taken, on the annotation layer
-- 0215 made for exactly this kind of thing. It has been counted on the
-- bill since it shipped, so it bills as "Sectionmark 2 no." on every
-- project where somebody placed one.
--
-- It is left alone deliberately, and not because it is right.
-- Correcting it REMOVES ROWS FROM BILLS PEOPLE HAVE ALREADY READ and
-- drops the totals beneath them — a visible change to an issued
-- document, which is a decision for whoever owns the commercial side
-- rather than something to slip in beside a new feature.
--
-- When that decision is made it is one line: add 'sectionmark' to the
-- exclusion list in the `points` CTE below, in a migration of its own
-- so it can be dated and pointed at. To see what it would change:
--
...
--
-- A Postgres function has no way to amend one line of one WHERE
-- clause, so it is replaced wholesale. This is 0212 with one role
-- added to the exclusion list, and nothing else changed — the same
-- shape 0212 itself has against 0207.
--
-- The NULL guard on the line above the list stays where it is. NULL
-- NOT IN (...) is NULL rather than true, so without it every point
-- with no role vanishes from the bill, which cost a whole class of
-- joints once. checkbomroles reads those two lines together for that
-- reason.
-- ════════════════════════════════════════════════════════════════
...
-- ── Check ───────────────────────────────────────────────────────
-- Expect no notes on the bill (section marks are still counted, and
-- deliberately so until somebody decides otherwise — see the head):
--   SELECT item, unit, quantity FROM gis_bom(<project>) WHERE unit = 'no.';
--
-- The points on the bill should be the drawing's points less its
-- seeds, span nodes, feeder end points, supplies and notes. The list
-- below is the same one the function uses, and the two have to stay in
-- step or this query stops testing anything. Expect no rows:
