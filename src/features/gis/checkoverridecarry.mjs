   ...
   functions, and to having no geometry key of its own left behind. */
{
  ...
  if (at < 0) fail("buildLvNetwork has gone");
    ...
    /* To the END of the function, not a fixed slice: adding a guard at
       the top of buildLvNetwork pushed the lines this looks for past a
       30,000-character window and the check reported them missing. That
       is fault 33, and this is its fifth outing. */
